...
# @blueprint.route('/user/level/query', menu_path='平台运营/会员/', methods=['GET', 'POST'])
# def api_platform_userlevel_query():
#     page_size = 10
#     form = {
...
#     }
#     levels = UserLevel.query(**kwargs)
#     # 查询总数
#     total_count = UserLevel.query(count=True, **kwargs)
#     total_pages = (total_count + page_size - 1) / page_size
#     return render_template('platform/user_level_query.html', form=form, levels=levels, statuses=dict(UserLevelStatus), page=form.get('next_page'), total_count=total_count, total_pages=total_pages)
#


@blueprint.route('/user/level/create', methods=['GET', 'POST'])
def api_platform_userlevel_create():
    if request.method=='GET':
        return render_template('platform/user_level_create.html', form={})
    else:
        form = {
        ...
        if not UserLevel.isvalid_value(form.get('value')):
            return render_template('platform/user_level_create.html', form=form, msg='级别值输入错误，必须是0-100间的数字！')
        if UserLevel.getbyvalue(form.get('value')):
            return render_template('platform/user_level_create.html', form=form, msg='该级别值已存在，请重新填写！')

        level = UserLevel(levelid=utils.uuid1())
        level.value = int(form.get('value'))
        level.name = form.get('name')
        level.alias = form.get('alias')
        level.status = UserLevelStatus.normal
        level.save()
        # qiyehao_message.send_text('【用户管理】\n\n级别，创建成功!\n\n名称：%s\n别名：%s\n级别值：%s' % (level.name, level.alias or '', level.value))
        return redirect('/%s/platform/user/level/update?levelid=%s&msg=%s' % (config.appname,level.levelid,  u'创建成功！'))


@blueprint.route('/user/level/update', methods=['GET', 'POST'])
def api_platform_userlevel_update():
    level = UserLevel(levelid=request.values.get('levelid'))
    if request.method=='GET':
        return render_template('platform/user_level_update.html', form={}, level=level, msg=request.values.get('msg') or '')
    else:
        form={
            'value': request.values.get('value'),
            'name': request.values.get('name'),
        ...
        if not UserLevel.isvalid_value(form.get('value')):
            return render_template('platform/user_level_update.html', form=form, level=level.to_dict(), msg='级别值输入错误，必须是0-100间的数字！')
        if level.value != int(form.get('value')) and UserLevel.getbyvalue(form.get('value')):
            return render_template('platform/user_level_update.html', form=form, level=level.to_dict(), msg='该级别值已存在，请重新填写！')
        ...
        level.save()
        # qiyehao_message.send_text('【用户管理】\n\n级别，修改成功!\n\n名称：%s\n别名：%s\n级别值：%s' % (level.name, level.alias or '', level.value))
        return redirect('/%s/platform/user/level/update?levelid=%s&msg=保存成功！' % (config.appname, level.levelid))


@blueprint.route('/user/level/delete', methods=['GET', 'POST'])
def api_platform_userlevel_delete():
    level = UserLevel(levelid=request.values.get('levelid'))
    level.value = int(time.time())
    level.status = UserLevelStatus.deleted
    level.save()
    # qiyehao_message.send_text('【用户管理】\n\n级别，删除成功!\n\n名称：%s\n别名：%s\n级别值：%s' % (level.name, level.alias or '', level.value))
    return redirect('/%s/platform/user/level/query?status=%s&next_page=%s' % (config.appname, request.values.get('stauts') or '', request.values.get('next_page') or ''))

# ====================================会员卡======================================
@blueprint.route('/vip/card/query', menu_path='平台运营/会员/会员卡管理', methods=['GET', 'POST'])
@needlogin
...
@blueprint.route('/vip/card/update', methods=['GET', 'POST'])
@needlogin
def api_platform_vip_card_update():
    if request.method=='GET':
        kwargs = {
            'status': 'published',
            'limit': 30,
        }
        products = Product.query(**kwargs)
        card = VipCard(cardid=request.values.get('cardid'))
        form = {
            'name': card.name,
            'num': card.num,
        ...
        return render_template('platform/vip_card_update.html', products=products, form=form)
    else:
        form = {
            'name': request.values.get('name'),
            'num': request.values.get('num'),
            'product': request.values.get('product'),
            'cardid': request.values.get('target_cardid'),
        }
        cardid = form.get('cardid')
        if not cardid:
            count = VipCard.query(count=True)
            card = VipCard(cardid= utils.uuid1())
            card.card_num = 'VIP'+str(count+1)
        else:
            card = VipCard(cardid=cardid)
        card.name = form.get('name')
        card.num = int(form.get('num'))
        card.productid = form.get('product')
        card.status = VipCardStatus.editing
        card.save()
        # qiyehao_message.send_text('【用户管理】\n\n级别，创建成功!\n\n名称：%s\n别名：%s\n级别值：%s' % (level.name, level.alias or '', level.value))
        return redirect('/%s/platform/vip/card/query' % (config.appname))

# 会员卡软删除
@blueprint.route('/vip/card/delete', methods=['GET', 'POST'])
@needlogin
def api_platform_vip_card_delete():
    card = VipCard(cardid= request.values.get('cardid'))
    if card.exists:
        card.status = VipCardStatus.deleted
...
def api_jiamei_vipcard_offset():
    ...
    if not vipcard.exists:
        raise BadRequest(description='该会员卡不存在！')
    if vipcard.status == VipCardStatus.published:
        vipcard.status = VipCardStatus.editing
        vipcard.save()
    return apphelper.format_ok_response()
...
def api_jiamei_vipcard_publish():
    ...
    if not vipcard.exists:
        raise BadRequest(description='该会员卡不存在！')
    if vipcard.status == VipCardStatus.editing:
        vipcard.status = VipCardStatus.published
        vipcard.save()
    return apphelper.format_ok_response()
...
def api_jiamei_vipcard_stick():
    ...
    vipcard.ot = int(time.time()*1000)
    vipcard.save()
    return apphelper.format_ok_response()

# ================================会员================================
@blueprint.route('/user/vip/query', methods=['GET', 'POST'])
@needlogin
def api_platform_user_vip_query():
    if request.method=='GET':
        ...
        for v in vipcards:
            ...
            if len(jiameiVip) > 0:
                ...
                v.opened = True,
                v.vipid = jiameiVip[0].vipid
            else:
                v.opened = False
                v.vipid = ''
            newcards.append(v)
        return render_template('platform/user_vip_update.html', uid=uid, vipcards=newcards)

@blueprint.route('/user/vip/update', methods=['GET', 'POST'])
@needlogin
def api_platform_user_vip_add():
        vipid = request.values.get('vipid')
        uid = request.values.get('uid')
        cardid = request.values.get('cardid')
        remain = request.values.get('remain')
        user = User(uid=uid)
        if not user.exists:
            return BadRequest(description='用户不存在')
        card = VipCard(cardid=cardid)
        if not card.exists:
            return BadRequest(description='该会员卡不存在')
        jiameiVip = JiameiVip(vipid=(vipid or utils.uuid1()))
        jiameiVip.remain = remain
        jiameiVip.uid = uid
        jiameiVip.cardid = cardid
        jiameiVip.save()
        return apphelper.format_ok_response(data={})

@blueprint.route('/user/vip/delete', methods=['GET', 'POST'])
@needlogin
def api_platform_user_vip_delete():
        vipid = request.values.get('vipid')
        if not vipid:
            return BadRequest(description='请求参数错误')
        jiameiVip = JiameiVip(vipid=vipid)
        jiameiVip.delete()
        return apphelper.format_ok_response(data={})
...
# ############### 微信公众号菜单修改 #####################
# @blueprint.route('/weixinmp/menus/set/query', menu_path='平台运营/微信公众号/公众号查询', methods=['GET', 'POST'])
# def api_weixinmp_menus_set_query():
#     return render_template('platform/weixinmp_menus_set_query.html', weixinmp=userconfig.weixin_mp_account)
#
# @blueprint.route('/weixinmp/menus/update', menu_path='平台运营/微信公众号/菜单更新', methods=['GET'])
# def api_weixinmp_menus_update():
#     thirdtype = request.values.get('thirdtype')
#     if not thirdtype:
#         raise BadRequest(description='请求参数错误！')
#     return render_template('platform/weixinmp_menus_update.html', thirdtype=thirdtype)
#
# @blueprint.route('/weixinmp/menus/get/ajax', methods=['POST', 'GET'])
# def api_weixinmp_menus_get_ajax():
#     thirdtype=request.values.get('thirdtype')
#     if not thirdtype:
#         raise BadRequest(description='请求参数错误！')
#     from xdock_user.lib.weixin import WeixinMpMenu
#     return apphelper.format_ok_response(data={'menus': WeixinMpMenu.get(thirdtype=thirdtype)})
#
# @blueprint.route('/weixinmp/menus/set/ajax', methods=['POST'])
# def api_weixinmp_menus_set_ajax():
#     menus = request.values.get('menus')
#     thirdtype = request.values.get('thirdtype')
#     if not thirdtype:
#         raise BadRequest(description='请求参数错误！')
#     from xdock_user.lib.weixin import WeixinMpMenu
#     WeixinMpMenu.set(json.loads(menus), thirdtype=thirdtype)
#     return apphelper.format_ok_response()
